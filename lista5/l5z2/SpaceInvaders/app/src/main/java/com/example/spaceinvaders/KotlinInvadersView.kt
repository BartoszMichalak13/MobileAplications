...
class KotlinInvadersView(
    ...
    private val db = Room.databaseBuilder(
        context,
        myRoomDB::class.java, "bestScoreDB"
...
//        Context.MODE_PRIVATE)
    ...
    // By def PlayerId = 1
    private var highScore = highestScoreInit() //prefs.getInt("highScore", 0)
    ...
    private fun highestScoreInit(): Int {
        var return_value: Int = -1
        //GlobalScope.launch {

            if (db.myDao().isEmpty()) {
                println("hello is empty")
                val TMPhs = HighestScore(1, 0)
                db.myDao().insertAll(TMPhs)
            }
            //return_value = db.myDao().getScorebyUid(1)
            var tmpList = db.myDao().getAll()
            return_value = tmpList[0].score!!
            println("hello? $return_value")
            for ( score in db.myDao().getAll() ){
                println("hello score $score")
            }
        //}
...
//            Context.MODE_PRIVATE)
        ...
        //GlobalScope.launch {
            var tmpList = db.myDao().getAll()
            val oldHighScore = tmpList[0].score//prefs.getInt("highScore", 0)

            if (highScore > oldHighScore!!) {
                println(" prev hello " + oldHighScore + " highest score " + highScore)
                //val TMPhighScore = HighestScore(1, highScore)
                db.myDao().update(highScore,1)
                println(" curr hello " + db.myDao().getAll()[0].score )
